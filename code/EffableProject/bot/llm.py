...
async def generate_followup_reaction(user_id: int, followup_answer_text: str) -> str:
    ...
    prompt = (
        "Пользователь рассказал, как прошла реализация его планов.\n"
        "Твоя задача — сформировать один короткий и естественный ответ пользователю на русском языке.\n\n"
        "Инструкция:\n"
        "1. Определи по сообщению пользователя, удалось ли ему реализовать свои планы.\n"
        "2. Если планы выполнены — искренне похвали пользователя.\n"
        "3. Если планы не выполнены — мягко поддержи его, без давления и морализаторства.\n"
        "4. Ответ должен состоять из 2–4 предложений.\n"
        "5. В конце ответа обязательно задай вопрос отдельной строкой:\n"
        "   «Кстати, какие планы на завтра?»\n\n"
        "Важно:\n"
        "- Сгенерируй только один итоговый ответ.\n"
        "- Не предлагай несколько вариантов и не используй формулировки вроде "
        "«если получилось» или «если не получилось».\n"
        "- Не добавляй пояснений, комментариев, разметки или списков.\n"
        "- Ответ должен быть готов к отправке пользователю без каких-либо изменений.\n\n"
...
async def generate_plans_today_reaction_and_ask_tomorrow(
    ...
    prompt = (
        ...
        "Твоя задача — сформировать один короткий ответ пользователю на русском языке.\n\n"
        "Инструкция:\n"
        "1. Определи по сообщению пользователя, удалось ли ему выполнить свои планы.\n"
        "2. Если планы выполнены — похвали пользователя.\n"
        "3. Если планы не выполнены — поддержи и подбодри его.\n"
        "4. Ответ должен состоять из 2–4 предложений.\n"
        "5. В конце ответа ОБЯЗАТЕЛЬНО задай вопрос отдельной строкой:\n"
        "   «Какие планы на завтра?»\n\n"
        "Важно:\n"
        ...
        "- Не используй формулировки вроде «если получилось» или «если не получилось».\n"
        "- Не добавляй пояснений, комментариев или разметки.\n"
        "- Ответ должен быть готов к отправке пользователю без изменений.\n\n"
        f"{plan_part}"
        f"Ответ пользователя:\n{plans_today_text}\n\n"
        "Итоговый ответ:"
    )
